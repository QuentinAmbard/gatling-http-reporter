  ...
  private def mapToGatlingConfig(config: Config) =
    GatlingConfiguration(
      ...
      data = DataConfiguration(
        ...
        http = HTTPDataWriterConfiguration(
          startUrl = config.getString(data.http.StartUrl),
          stopUrl = config.getString(data.http.StopUrl),
          reportUrl = config.getString(data.http.ReportUrl),
          light = config.getBoolean(data.http.Light),
          writePeriod = config.getInt(data.http.WritePeriod) seconds
        )
      ),
      // [fl]
      //
...
case class HTTPDataWriterConfiguration(
    startUrl:    String,
    stopUrl:     String,
    reportUrl:   String,
    light:       Boolean,
